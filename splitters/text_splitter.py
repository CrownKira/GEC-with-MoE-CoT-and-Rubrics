...
class SemanticChunker(BaseDocumentTransformer):
    ...
    def split_text(self, text: str) -> List[str]:
        # Splitting the text on newline character '\n'
        single_sentences_list = text.split("\n")

        # Early return if there's only one sentence (or none), as there's nothing to chunk
        ...
        # Determine the appropriate breakpoint threshold based on the configuration
        ...
        # Iterate through the indices where distances exceed the threshold to form chunks
        for index in indices_above_thresh:
            end_index = index
            group = sentences[start_index : end_index + 1]
            combined_text = "\n".join(
                [d["sentence"] for d in group]
            )  # Use newline to combine sentences into a chunk
            chunks.append(combined_text)
            start_index = index + 1

        # Handling the final chunk after the last threshold exceedance, if any
